                ...
                @if (Auth::user()->id !== $post->user->id)
                    <span style="float:left;">
                        <a href="#" class="like" data-postid="{{ $post->id }}">{{ Auth::user()->likes()->where('post_id', $post->id)->first() ?
                            Auth::user()->likes()->where('post_id', $post->id)->first()->state == true ? 'شما این پست رو دوست داشتید' : 'دوست داشتم' : 'دوست داشتم' }}</a> |  
                        <a href="#" class="like" data-postid="{{ $post->id }}">{{ Auth::user()->likes()->where('post_id', $post->id)->first() ?
                        Auth::user()->likes()->where('post_id', $post->id)->first()->state == false ? 'شما این پست رو دوست نداشتید' : 'دوست نداشتم' : 'دوست نداشتم' }}</a>    
                    </span>
                @endif
            @endauth
                <hr style="margin-top: 40px;">    
            </span>
            <h2 class="card-title" style="color:blue">{{ $post->title }}</h2>
            <p class="card-text">
                        ...
                        {{ Form::open(['action' => ['CommentsController@destroy' , $item->id], 'method' => 'DELETE']) }}
                            <span class="float-left mx-auto"> 
                                <a class="tooltips" data-toggle="tooltip" data-placement="top" title="Delete">
                                    <button type="submit" onclick="return confirm('این کامنت رو حذف می کنید؟');" style="border: 0; background: none;">
